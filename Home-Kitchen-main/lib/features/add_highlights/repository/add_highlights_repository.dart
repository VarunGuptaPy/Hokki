...
class AddHighlightsRepository {
  ...
  Future<void> addHighlightsToDatabase(
      String title, String description, File file, BuildContext context) async {
    String idOfImage = const Uuid().v1();
    String idOfPost = DateTime.now().microsecondsSinceEpoch.toString();
    String downloadUrl = await saveImageToFirebaseStorage(
        file: file, path: 'posts/$idOfImage', context: context);
